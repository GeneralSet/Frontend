...
import { CardData, GeneratedDeckMetaData } from "deckBuilder/features";
import { DeckMetaData } from "deckBuilder/types";

interface PresetDeckConfig {
  options: DeckMetaData;
  path: string;
  ext: "svg" | "png";
  name: string;
}

interface SymbolDeckConfig {
  options: GeneratedDeckMetaData;
  defaultCardData?: Partial<CardData>;
  name: string;
}

const StableDiffusion: PresetDeckConfig[] = [
...
const Symbols: SymbolDeckConfig[] = [
  ...
  {options: {colors: ['Blue', 'Green', 'Purple'], filters: ["crinkle", "ink", "blur"], numbers: [1,3,5]}, defaultCardData: {shapes: "Triangle"}, name: "Filters"},
  {options: {shapes: ['Tracks - Deer', 'Tracks - Wolf', 'Tracks - Frog'], colors: ['Brown', 'Green', 'Cyan'], numbers: [2,5,8]},  name: "Tracks"},
  {options: {shapes: ['Triangle', 'Circle - Semi', 'Tetris - S Block'], colors: ['Maroon', 'Teal', 'Navy'], patterns: ["striped", "gradient", "triangles"]},  name: "Patterns"},
]
...
export const Menu: React.FC = () => {
  ...
  const setPreSetDeck = (d: PresetDeckConfig) => {
    dispatch(actions.updateDeck({deck: new PresetDeck(d.options, d.path, d.ext)}));
  }

  const setGeneratedDeck = (d: SymbolDeckConfig) => {
